    ...
    while (!stop && (iter < iterations)) {
        ...
        while (!oneStepSuccess)  // 不断尝试 Lambda, 直到成功迭代一步
        {
            ...
            if(!oneStepSuccess){
                std::cout << "iter: " << iter << " , chi= " << currentChi_ << " , Lambda= " << currentLambda_
                      << std::endl;
                ofstream save_lamda_false;
                save_lamda_false.open("../../data/Lamda.txt", ios::app);
                save_lamda_false << iter << "\t" << currentLambda_ << endl;
                save_lamda_false.close();
            }
            */
...
bool Problem::IsGoodStepInLM_1() {
    ulong size = Hessian_.cols();
    assert(Hessian_.rows() == Hessian_.clos() && "Hessian is not square");
    ...
    if (rho > 0 && isfinite(tempChi))   // last step was good, 误差在下降
    {
        //论文中第一个策略
        // L取9
        double alpha = currentLambda_ * 1.0 / 9.0;
        currentLambda_ = (std::max)(alpha, 10e-7);
        currentChi_ = tempChi;
        ...
        //论文中第一个策略
        // L取11
        double alpha = currentLambda_ * 11.0;
        currentLambda_ = (std::min)(alpha, 10e+7);
...
bool Problem::IsGoodStepInLM_2() {
    ...
    if (rho > 0 && isfinite(tempChi))   // last step was good, 误差在下降
    {
        currentLambda_ = (std::max)(currentLambda_ / (1 + alpha), 10e-7);
        currentChi_ = tempChi;
        return true;
    } else {
