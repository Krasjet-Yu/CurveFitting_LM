...
int main()
{
    ...
    // 构建 problem，普通类型的problem
    Problem problem(Problem::ProblemType::GENERIC_PROBLEM);
    // 构建顶点集合，顶点就是待优化的变量abc，因此CurveFittingVertex是一个Vec3d类型
    shared_ptr< CurveFittingVertex > vertex(new CurveFittingVertex());

    // 设定待估计参数 a, b, c初始值，在certex.h文件中定义了SetParameters()函数，{VecX parameters_ = parameters};
